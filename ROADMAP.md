...
- **Agente C — Social** 🟢 (migrations aplicadas 2026-07-04; social VIVO)
  ...
  - 🟡 **Presence ao vivo ("X focando agora")** — código completo (`presence_service.dart` + `circlePresenceProvider` autoDispose + linha com dot pulsante no círculo, honra reduce-motion, best-effort). Publica a fase de foco do usuário no canal `circle:{id}`. analyze + 23 testes ok. **Verificação runtime pendente:** 2 devices + confirmar Realtime ligado no projeto Supabase.
